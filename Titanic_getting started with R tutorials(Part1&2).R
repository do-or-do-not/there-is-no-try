...
# 'Getting started with R' Part 1 and Part 2. It can be found at:
...
# This submission got a Kaggle score of 0.622
...
# We can see that 3rd class woman don't do too good

test$Survived <- 0
test$Survived[test$Sex == 'female'] <- 1
# again all woman set to survived.
test$Survived[test$Sex == 'female' & test$Pclass == 3 & test$Fare >= 20] <- 0
# all 3rd class woman set to 'not survive' when fair is greater than 20

submit_allwomanbut3rdclass <- data.frame(PassengerId = test$PassengerId, Survived = test$Survived)

write.csv(submit_allwomanbut3rdclass, file = "womanfirst_not3rdclass.csv", row.names = FALSE)

# This submission got a score of 0.779 on Kaggle
